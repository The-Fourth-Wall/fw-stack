...
class McpClientManager {
  ...
  async connectServer(config) {
    if (this.connections.has(config.name)) {
      log2.debug("Server already connected, skipping", { server: config.name });
      return;
    }
    if (!this.deps) {
      try {
        ...
        log2.warn("Failed to load MCP SDK", { error: String(err) });
        return;
      }
    }
    ...
    try {
      ...
      log2.warn("MCP server connection failed", {
        server: config.name,
        error: String(err)
      });
      return;
    ...
    try {
      ...
      tools = result?.tools ?? [];
      log2.info("MCP server connected", {
        server: config.name,
        tools: tools.length
      });
    } catch (err) {
      log2.warn("MCP tool discovery failed", {
        server: config.name,
        error: String(err)
      });
    }
...
`);
      ...
      log2.warn("MCP tool call failed", {
        server: serverName,
        tool: toolName,
  ...
  async disconnectAll() {
    for (const [name, conn] of this.connections) {
      try {
        ...
        log2.debug("MCP server disconnected", { server: name });
      } catch (err) {
        log2.debug("MCP server disconnect failed", { server: name, error: String(err) });
      }
    }
...
var log2, defaultDeps = null;
var init_client_manager = __esm(() => {
  init_logger();
  log2 = createLogger("mcp:client-manager");
});

// src/cli/mcptool.ts
init_config();
init_client_manager();
var USAGE = `mcptool — call MCP server tools from the shell
...
async function main() {
  ...
  if (command === "tools") {
    const filter = args[1];
    const toConnect = filter ? configs.filter((c) => c.name === filter) : configs;
    ...
    if (tools.length === 0) {
      ...
      for (const t of tools) {
        const params = t.inputSchema ? Object.keys(t.inputSchema.properties ?? {}).join(", ") : "";
